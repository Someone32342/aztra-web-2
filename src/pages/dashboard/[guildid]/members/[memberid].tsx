...
const MemberDashboard: NextPage<MemberDashboardRouteProps> = ({
  ...
  const { data: member } = useSWR<MemberExtended, AxiosError>(
    new Cookies().get('ACCESS_TOKEN')
      ? urljoin(api, `/discord/guilds/${guildId}/members/${memberId}`)
      : null,
    (url) =>
      axios
        .get(url, {
  ...
  const { data: exps } = useSWR<Exp[], AxiosError>(
    new Cookies().get('ACCESS_TOKEN')
      ? urljoin(api, `/servers/${guildId}/exps`)
      : null,
    (url) =>
      axios
        .get(url, {
  ...
  let expIndex = exps
    ...
    .findIndex((m) => m.id === member?.user.id);
    ...
    .map((one) => (
      <Card key={one.uuid} bg="dark" className="mb-2 shadow-sm shadow">
        <Card.Body as={Row} className="py-1 d-flex justify-content-between">
          ...
          <Col
            xs={3}
            className="d-flex align-items-center my-0 justify-content-end"
          >
            <div
            ...
            >
              <div className="text-right">{one.count}회</div>
    ...
    .map((one) => (
      <Card key={one.uuid} bg="dark" className="mb-2 shadow-sm shadow">
        <Card.Body as={Row} className="py-1 d-flex justify-content-between">
          <Col xs={9} className="d-flex">
            <div className="my-auto d-inline-block text-truncate">
              {ticketsets?.find((o) => o.uuid === one.setuuid)?.name}#
              {one.number}
            </div>
          </Col>
          <Col
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {reqExp ? numberWithCommas(reqCompleted) : '--'}/
                              {reqExp ? numberWithCommas(reqExp) : '--'} P
                            </div>
                          </div>
                        </CircularProgressbarWithChildren>
                      </div>
                  ...
                  <Col className="pb-5" xs={12} xl={6}>
                    <div className="d-flex justify-content-between">
                      ...
                      <div>
                        <Link
                          passHref
                          href={`/dashboard/${guildId}/warns/list?search=${encodeURIComponent(
                            member.user.tag || ''
                          )}&type=target`}
                        >
                          <Button
