...
const Statistics: NextPage<StatisticsProps> = ({ guildId }) => {
  const { data: memberCounts } = useSWR<MemberCount[], AxiosError>(
    new Cookies().get('ACCESS_TOKEN')
      ? urljoin(api, `/servers/${guildId}/statistics/membercounts`)
      : null,
    (url) =>
      axios
        .get(url, {
  ...
  const { data: msgCounts } = useSWR<MsgCount[], AxiosError>(
    new Cookies().get('ACCESS_TOKEN')
      ? urljoin(api, `/servers/${guildId}/statistics/msgcounts`)
      : null,
    (url) =>
      axios
        .get(url, {
              ...
              <>
                <Row className="dashboard-section">
                  <div>
                    <h3>통계</h3>
                    <div className="py-2">
                      서버의 각종 통계를 보여줍니다. Aztra가 초대된 이후에 정보
                      수집이 시작됩니다. 메시지 통계는 Aztra에{' '}
                      <b>메시지 읽기</b> 권한이 있는 채널만 수집됩니다.
                    </div>
                  </div>
                </Row>
