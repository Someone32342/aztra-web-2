...
export default function Servers() {
  ...
  const { data, error, mutate } = useSWR<PartialGuildExtend[], AxiosError>(
    new Cookies().get('ACCESS_TOKEN')
      ? urljoin(api, '/discord/users/@me/guilds')
      : null,
    (url) =>
      axios
        .get(url, {
          ...
          <Container>
            <Row>
              <Col>
                ...
                >
                  {one.icon && (
                    <img
                      alt={one.name}
                      src={`https://cdn.discordapp.com/icons/${one.id}/${one.icon}.png`}
                      style={{
                        maxHeight: 40,
                        marginRight: 15,
                        borderRadius: '70%',
              ...
              <Col className="d-flex align-items-center justify-content-end">
                {one.bot_joined ? (
                  <Link href={`/dashboard/${one.id}`} shallow={true} passHref>
                    <Button
                      className="d-flex align-items-center"
                      variant="aztra"
                      size="sm"
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const left =
                        (width - w) / 2 / systemZoom + dualScreenLeft;
                      const top = (height - h) / 2 / systemZoom + dualScreenTop;
                      const newWindow = window.open(
                        links.invite_with_redirect[process.env.NODE_ENV] +
                          `&guild_id=${one.id}&disable_guild_select=true`,
                        'Aztra 초대하기',
                        `
                        width=${w / systemZoom},
                        height=${h / systemZoom}, 
    ...
    <>
      ...
      <Layout>
          ...
          <Container fluid="sm">
            {!(data || error) ? (
              ...
              >
                ...
                />
                <h4 className="pt-5">서버 목록을 가져오고 있습니다...</h4>
              </div>
            ) : (
              guild_cards
            )}
        ...
        <Modal
          ...
          onHide={() => {}}
        >
          <Modal.Body>서버 정보를 가져오는 데 실패했습니다!</Modal.Body>
          <Modal.Footer>
