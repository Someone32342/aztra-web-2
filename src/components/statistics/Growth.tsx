...
const Growth: React.FC<GrowthProps> = ({ memberCounts, msgCounts }) => {
  ...
  const memberCountsCSVDownload = () => {
    let csvData =
      ...
      Days?.filter(
        (o) =>
          dayjs.utc(o.split('T')[0]) <= dayjs(new Date().setHours(0, 0, 0, 0))
      )
        .map(
          (o) =>
            `${dayjs.utc(o).local().format('DD일')}, ${
              memberCounts.find(
                (one) => dayjs.utc(one.dt).local().format('YYYY-MM-DD') === o
              )?.count ?? 0
            }`
        )
        .join('\n');
  ...
  const msgCountsCSVDownload = () => {
    let csvData =
      ...
      Days?.filter(
        ...
        .map(
          (o) =>
            `${dayjs.utc(o).local().format('DD일')}, ${msgCounts
              ?.filter((a) => a.dt.split('T')[0] === o)
              .reduce(
                (a, b) => a + b.count_user + (includeBot ? b.count_bot : 0),
                0
              )}`
        )
        .join('\n');
          ...
          const counts = msgCounts
            ?.filter((a) =>
              dayjs
                .utc(a.dt)
                .isSame(
                  dayjs.utc(new Date().setHours(0, 0, 0, 0)).add(hours, 'hours')
                )
            )
            .reduce(
              (a, b) => a + b.count_user + (includeBot ? b.count_bot : 0),
  ...
  const Days = Array.from(Array(DAYLIMIT_DEFAULT).keys()).map((n) =>
    dayjs()
      .subtract(DAYLIMIT_DEFAULT - n, 'days')
      .format('YYYY-MM-DD')
  );
          ...
          <div className="d-sm-flex align-items-center mb-2">
            ...
            <div className="ml-auto d-flex my-2 my-sm-auto">
              ...
              <Button
                ...
                style={{ wordBreak: 'keep-all' }}
                size="sm"
                onClick={memberCountsCSVDownload}
              >
                <AssessmentIcon />
              </Button>
            </div>
                ...
                datasets: [
                  {
                    ...
                    data: Days?.filter(
                      ...
                      (o) =>
                        memberCounts.find(
                          (one) =>
                            dayjs.utc(one.dt).local().format('YYYY-MM-DD') === o
                        )?.count ?? 0
                    ),
                  },
                ],
                ...
                datasets: [
                  {
                    ...
                    data: Days?.filter(
                      ...
                      msgCounts
                        ...
                        .reduce(
                          (a, b) =>
                            a + b.count_user + (includeBot ? b.count_bot : 0),
                          0
                        )
                    ),
                  },
          ...
          <div className="d-sm-flex align-items-center mb-2">
            ...
            <div className="ml-auto d-flex my-2 my-sm-auto">
              ...
              <Button
                ...
                style={{ wordBreak: 'keep-all' }}
                size="sm"
                onClick={msgTimeLineCSVDownload}
              >
                <AssessmentIcon />
              </Button>
            </div>
                ...
                datasets: [
                  {
                    ...
                    data: Array.from(Array(47).keys()).map((o) => {
