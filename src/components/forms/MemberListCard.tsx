...
const MemberListCard: React.FC<MemberListCardProps> = memo(
  ({ member, guild }) => {
    return (
      <Card as={Container} fluid bg="dark" className="mb-2 shadow">
        <Card.Body as={Row} className="flex-column flex-sm-row py-1">
          ...
          <Col
            sm={4}
            className="d-flex justify-content-sm-end align-items-center px-0"
          >
            <Link
              href={`/dashboard/${guild.id}/members/${member.user.id}`}
              passHref
            >
