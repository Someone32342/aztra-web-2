...
class MemberDashboard extends Component<MemberDashboardRouteProps & WithRouterProps, MemberDashboardState> {
  ...
  getMember = async (token: string) => {
    try {
      let res = await axios.get(`${api}/discord/guilds/${this.props.guildId}/members/${this.props.memberId}`, {
        headers: {
                  ...
                  <Row>
                    <Col className="pb-5" xs={12} xl={6}>
                      <div className="d-flex justify-content-between">
                        ...
                        <div>
                          <Link href={`/dashboard/${this.props.guildId}/warns/list?search=${encodeURIComponent(member?.user.tag || '')}&type=target`}>
                            <Button hidden={!this.state.warns?.length} variant="aztra" size="sm">
                              더보기
                          </Button>
                          </Link>
                        </div>
